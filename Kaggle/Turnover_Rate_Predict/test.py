...
data['YearsAtCompany_Less_One'] = [1 if value <=1 else 0 for value in data['YearsAtCompany']]
...
def model_ensemble(x_train, y_train, x_test, y_test):
    ...
    #scores = cross_val_score(lr, x_all_train, y_train, cv=10)
    #print("LR mean cv(10) scores = {}".format(np.mean(scores)))
    ...
    for index, clf in enumerate(clfs):
        #print("{} test on test data".format(clf))
    ...
    y_pre = y_pre[:, 1].reshape(-1, 1)
    plot_roc_curve(y_test, y_pre)
    cross_validation(lr, x_all_test, y_test, 10)
    # #
